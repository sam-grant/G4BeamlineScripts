...
then
    ...
    echo "Submit a test job:   ./submit_grid.sh -1 Mu2e.in"
...
if [ $1 == 0 ]; then
    command="mu2eg4bl --in=${mu2ein} --tar=${tarbal} --njobs=200 --events-per-job=5000 --g4bl-version=v3_06 --g4bl-add-args='READ_Beam_File=$1' ${opt_offsite}"
elif [ $1 == 1 ]; then
